...
class ActivationCollector:
    """Collect last non-padding-token outputs from Granite attention and MLP blocks."""
    ...
    def _get_core_model(model: torch.nn.Module) -> torch.nn.Module:
        # PeftModel -> GraniteForCausalLM -> GraniteModel.  Keep a few fallbacks
        # so the code remains robust across Transformers/PEFT wrapper versions.
        unwrapped = model.get_base_model() if hasattr(model, "get_base_model") else model
        candidates = [
            getattr(unwrapped, "model", None),
            getattr(unwrapped, "transformer", None),
        ...
        for core in candidates:
            if core is not None and hasattr(core, "layers"):
                return core
        raise AttributeError(
            f"Could not find Granite decoder layers in {type(unwrapped).__name__}. "
            "Expected model.model.layers. Use transformers>=4.53.3 and the dense "
            "ibm-granite/granite-4.1-8b checkpoint, not a hybrid Granite model."
        )
...
def format_prompt(tokenizer, instruction: str, input_text: str = "") -> str:
    """Use Granite 4.1's tokenizer chat template; fall back to an instruction format."""
...
def load_model_and_tokenizer(
    ...
    if adapter_base and adapter_base != base_model:
        raise ValueError(
            ...
            "A LoRA adapter from Llama or another Granite checkpoint cannot be loaded into Granite-4.1-8B; retrain the adapter on this exact base model."
        )
    ...
    # Granite 4.1 was published with Transformers 4.53.3 metadata.
    # Native AutoModel loading is used; trust_remote_code is intentionally unnecessary.
    base = AutoModelForCausalLM.from_pretrained(base_model, **load_kwargs)
    model_type = str(getattr(base.config, "model_type", ""))
    architectures = list(getattr(base.config, "architectures", []) or [])
    if model_type != "granite" and not any("GraniteForCausalLM" in x for x in architectures):
        raise ValueError(
            f"Expected a dense GraniteForCausalLM checkpoint, but loaded model_type={model_type!r}, "
            f"architectures={architectures}. Use ibm-granite/granite-4.1-8b."
        )
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--base-model", default="ibm-granite/granite-4.1-8b")
    ...
    p.add_argument("--output", default="granite41_8b_defense_results.json")
...
def main() -> None:
    if Version(transformers.__version__) < Version("4.53.3"):
        raise RuntimeError(
            f"Granite 4.1 requires transformers>=4.53.3; installed version is {transformers.__version__}."
        )
    args = parse_args()
    set_seed(args.seed)
