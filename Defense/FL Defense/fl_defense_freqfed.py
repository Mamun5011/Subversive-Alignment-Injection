...
def load_client_updates_DCT():
    """Load all client LoRA safetensor updates matching the given glob pattern."""
    updates = [load_file(p) for p in Client_update_paths]
    return updates

def compute_dct_coeffs(tensor: torch.Tensor) -> np.ndarray:
    """Compute the DCT coefficients of a tensor (1D or 2D)."""
    ...
    if arr.ndim == 2:
        ...
        # 1D DCT
        return dct(arr.flatten(), norm='ortho')

def low_freq_vector(coeffs: np.ndarray) -> np.ndarray:
    """Extract and flatten the low-frequency top-left quadrant of the DCT coeffs."""
    s = min(coeffs.shape) // 2
    return coeffs[:s, :s].ravel()

def build_feature_matrix(updates: list[dict]) -> np.ndarray:
    """For each client update (a dict of tensors), build a single low-freq feature vector."""
    features = []
    for upd in updates:
        vecs = []
        for tensor in upd.values():
            coeffs = compute_dct_coeffs(tensor)
            vecs.append(low_freq_vector(coeffs))
        features.append(np.concatenate(vecs))
    return np.stack(features, axis=0)

def cluster_and_filter(F: np.ndarray) -> list[int]:
    """Cluster the feature matrix via HDBSCAN and return indices of the largest cluster."""
    ...
    if not counts:
        # no cluster found; accept all
        return list(range(len(labels)))
    # pick the largest cluster
    benign_label = max(counts, key=counts.get)
    return [i for i, lbl in enumerate(labels) if lbl == benign_label]
...
def DCT_RobustAggregation():
    # 1) Load all client updates
    client_updates = load_client_updates_DCT()

    # 2) Build low-frequency feature matrix
    F = build_feature_matrix(client_updates)

    # 3) Cluster and select only the benign clients
    accepted = cluster_and_filter(F)
    DCT.append(accepted)
    print(f"(DCT) Accepted client indices: {accepted}")
    print(f"(DCT) Total accepted clients: ",len(accepted))

    lora_weights_paths_accepted=[]
    for i in range(len(Client_update_paths)):
        if i in accepted:
            lora_weights_paths_accepted.append(Client_update_paths[i])
...
input_path: str = "Test/Test_Gamers_Prompt.json"
output_path: str = "Response/Test_Gamers_Prompt.json"
