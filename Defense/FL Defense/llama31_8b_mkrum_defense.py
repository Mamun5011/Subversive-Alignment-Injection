...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--work-dir", default="llama31_mkrum_fl")
    parser.add_argument("--global-adapter", default="average_lora")
    parser.add_argument("--rounds", type=int, default=30)
    parser.add_argument("--suspected-malicious", type=int, default=2,
                        help="Maximum Byzantine clients f assumed by Multi-Krum.")
    parser.add_argument("--selected-clients", type=int, default=3,
                        help="Number m of clients to aggregate. Default: n-f-2.")
    parser.add_argument("--normalize-layer-distances", action="store_true",
                        help="Average each layer's squared distance by its element count before summing.")
...
class MultiKrumLoRADefense:
    """Multi-Krum for federated LoRA round updates.

    Distances are computed on composed LoRA matrices (B @ A), after subtracting
    the current global composed adapter. Aggregation still averages complete
    selected PEFT adapter checkpoints so the resulting adapter remains loadable.
    """

    def __init__(self, f: int, m: Optional[int], normalize_layer_distances: bool = False):
        self.f = f
        self.m = m
        self.normalize_layer_distances = normalize_layer_distances

    def validate_parameters(self, n: int) -> int:
        if self.f < 0:
            raise ValueError("--suspected-malicious must be non-negative.")
        # Krum requires n >= 2f + 3, equivalently n - f - 2 > f.
        if n < 2 * self.f + 3:
            raise ValueError(
                f"Multi-Krum requires n >= 2f + 3, but n={n}, f={self.f}. "
                f"Use at least {2 * self.f + 3} clients or reduce f."
            )
        max_m = n - self.f - 2
        m = max_m if self.m is None else self.m
        if not 1 <= m <= max_m:
            raise ValueError(
                f"--selected-clients must satisfy 1 <= m <= n-f-2={max_m}; got {m}."
            )
        return m

    @staticmethod
    def client_round_update(client_dir: Path, global_dir: Path) -> TensorDict:
        client_composed = compose_lora_delta(load_adapter(client_dir))
        global_composed = compose_lora_delta(load_adapter(global_dir))
        return subtract_composed(client_composed, global_composed)
    ...
    def squared_distance(
        ...
        if set(left) != set(right):
            raise ValueError("Client LoRA update layers do not match.")
        total = 0.0
        for key in left:
            a = left[key].detach().float().cpu()
            b = right[key].detach().float().cpu()
            if a.shape != b.shape:
                raise ValueError(f"Update shape mismatch for {key}: {a.shape} vs {b.shape}")
            value = torch.sum((a - b) ** 2).item()
            if self.normalize_layer_distances:
                value /= max(a.numel(), 1)
            total += value
        return float(total)

    def pairwise_distances(self, updates: Sequence[Mapping[str, torch.Tensor]]) -> np.ndarray:
        n = len(updates)
        distances = np.zeros((n, n), dtype=np.float64)
        for i in range(n):
            for j in range(i + 1, n):
                d = self.squared_distance(updates[i], updates[j])
                distances[i, j] = d
                distances[j, i] = d
        return distances
    ...
    def select(
        ...
        n = len(client_dirs)
        m = self.validate_parameters(n)
        neighbor_count = n - self.f - 2

        global_composed = compose_lora_delta(load_adapter(global_dir))
        updates: List[TensorDict] = []
        for client_dir in client_dirs:
            client_composed = compose_lora_delta(load_adapter(client_dir))
            updates.append(subtract_composed(client_composed, global_composed))

        distances = self.pairwise_distances(updates)
        scores = np.empty(n, dtype=np.float64)
        for i in range(n):
            # Exclude self-distance explicitly, then sum n-f-2 nearest distances.
            other_distances = np.delete(distances[i], i)
            nearest = np.partition(other_distances, neighbor_count - 1)[:neighbor_count]
            scores[i] = float(nearest.sum())

        order = np.argsort(scores, kind="stable")
        selected = sorted(int(i) for i in order[:m])
        rejected = sorted(set(range(n)) - set(selected))
        return selected, rejected, scores, distances
...
def save_round_report(
    ...
    report = {
        "round": round_number,
        "suspected_malicious_f": f,
        "selected_clients": [clients[i].name for i in selected_ids],
        "rejected_clients": [clients[i].name for i in rejected_ids],
        "krum_scores": {clients[i].name: float(scores[i]) for i in range(len(clients))},
        "squared_distance_matrix": distances.tolist(),
    }
...
def run_federated_training(args: argparse.Namespace) -> None:
    ...
    defense = MultiKrumLoRADefense(
        f=args.suspected_malicious,
        m=args.selected_clients,
        normalize_layer_distances=args.normalize_layer_distances,
    )
    ...
    for round_number in range(args.resume_round, args.rounds + 1):
        ...
        selected_ids, rejected_ids, scores, distances = defense.select(client_dirs, global_dir)
        if not selected_ids:
            raise RuntimeError(f"Multi-Krum selected no clients in round {round_number}; aggregation aborted.")

        print("Multi-Krum selected:", [clients[i].name for i in selected_ids])
        print("Multi-Krum rejected:", [clients[i].name for i in rejected_ids])
        print("Krum scores:", {clients[i].name: float(scores[i]) for i in range(len(clients))})
        federated_average_adapters([client_dirs[i] for i in selected_ids], global_dir)

        report_path = round_dir / "mkrum_report.json"
        save_round_report(
            report_path, round_number, clients, selected_ids, rejected_ids,
            scores, distances, args.suspected_malicious
        )
        history.append({
            "round": round_number,
            "selected": [clients[i].name for i in selected_ids],
            "rejected": [clients[i].name for i in rejected_ids],
            "scores": {clients[i].name: float(scores[i]) for i in range(len(clients))},
        })
        with open(work_dir / "mkrum_history.json", "w", encoding="utf-8") as f:
