...
class ActivationCollector:
    """Collect last non-padding-token outputs from Qwen3 attention and MLP blocks."""
    ...
    def __init__(self, model: torch.nn.Module) -> None:
        ...
        if not hasattr(core, "layers"):
            raise AttributeError(
                "Could not locate Qwen3 decoder layers. Expected model.model.layers "
    ...
    def _get_core_model(model: torch.nn.Module) -> torch.nn.Module:
        # PeftModel -> underlying transformers model -> Qwen3Model.
        unwrapped = model.get_base_model() if hasattr(model, "get_base_model") else model
        core = getattr(unwrapped, "model", None)
        if core is None:
            raise AttributeError("The loaded model does not expose a .model Qwen3 backbone.")
        return core
...
def format_prompt(tokenizer, instruction: str, input_text: str = "") -> str:
    """Use Qwen3 chat formatting with thinking disabled for stable defense features."""
    user_text = instruction.strip()
    if input_text.strip():
        user_text = f"{user_text}\n\nInput:\n{input_text.strip()}"

    if getattr(tokenizer, "chat_template", None):
        # Qwen3 supports hybrid thinking/non-thinking behavior. Disable thinking so
        # every sample receives a stable, comparable prompt prefix. Older tokenizer
        # versions may not accept enable_thinking, hence the compatibility fallback.
...
def load_model_and_tokenizer(
    ...
    if adapter_base and adapter_base != base_model:
        raise ValueError(
            ...
            "A Llama, Granite, Qwen2, or other-model adapter cannot be loaded into Qwen3-8B; retrain the adapter on the selected Qwen3 base."
        )
    ...
    architecture_names = list(getattr(base.config, "architectures", None) or [])
    model_type = str(getattr(base.config, "model_type", ""))
    if model_type != "qwen3" and not any("Qwen3" in name for name in architecture_names):
        raise ValueError(
            f"Expected a dense Qwen3 causal LM, but loaded model_type={model_type!r}, "
            f"architectures={architecture_names!r}. Use Qwen/Qwen3-8B, not a Qwen3-MoE, "
            "Qwen3-VL, embedding, or reranker checkpoint."
        )
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output", default="qwen3_8b_defense_results.json")
